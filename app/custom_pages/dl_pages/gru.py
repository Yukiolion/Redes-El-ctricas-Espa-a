...
def gru(_):
    ...
    X_TEST_PATH = os.path.join(MODEL_DIR, "X_test_gru.pkl")
    Y_TEST_PATH = os.path.join(MODEL_DIR, "y_test_gru.pkl")
    ...
    st.write("En esta gráfica podemos ver los datos de la demanda en azul frente a los datos predichos en rojo. " \
            "Si tenemos en cuenta las métricas, podemos ver que tiene una buena aproximación con los valores reales de la demanda.")
    ...
    with col2:
        st.dataframe(df_metricas2.style.format({
            "MAE": "{:.2f}",
            "RMSE": "{:.2f}",
            "R²": "{:.4f}"
    ...
    st.write("Esta gráfica muestra la evolución de la pérdida durante el entrenamiento del modelo GRU. Aquí podemos observar que el modelo " \
    "va aprendiendo a lo largo de las 35 épocas.")
    ...
    st.session_state["df_metricas2"] = df_metricas2
